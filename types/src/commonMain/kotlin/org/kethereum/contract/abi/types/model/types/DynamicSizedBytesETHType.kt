...
class DynamicSizedBytesETHType(override val paddedValue: ByteArray) : ETHType<ByteArray> {
    ...
    override fun toKotlinType(): ByteArray {
        ...
        return input.getBytes(len!!.toKotlinType().intValue())
    }
    ...
    companion object {
        fun ofPaginatedByteArray(input: PaginatedByteArray): DynamicSizedBytesETHType? {
            ...
            input.jumpTo(pos.intValue())
            val len = UIntETHType.ofPaginatedByteArray(input, BitsTypeParams(256)) ?: return null

            val array = len.toPaged().content + input.getBytes(len.toKotlinType().intValue())
                .rightPadToFixedPageSize(ETH_TYPE_PAGESIZE)
            input.endJump()
            return DynamicSizedBytesETHType(array)
        }

        fun ofNativeKotlinType(input: ByteArray) =
            DynamicSizedBytesETHType(UIntETHType.ofNativeKotlinType(input.size.toBigInteger(),
                BitsTypeParams(256)).paddedValue + input.rightPadToFixedPageSize(ETH_TYPE_PAGESIZE))
