...
fun String.toRLP() = RLPElement(encodeToByteArray())

fun Int.toRLP() = RLPElement(toMinimalByteArray())
fun BigInteger.toRLP() = RLPElement(toByteArray())
...
fun RLPElement.toUnsignedBigIntegerFromRLP(): BigInteger =
    if (bytes.isEmpty()) ZERO else BigInteger.fromByteArray(bytes, Sign.POSITIVE)
...
fun RLPElement.toStringFromRLP() = bytes.decodeToString()
